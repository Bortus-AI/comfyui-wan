...
# Install PyTorch stable instead of nightly (much smaller and more reliable)
RUN --mount=type=cache,target=/root/.cache/pip \
    pip install --no-cache-dir torch torchvision torchaudio \
        --index-url https://download.pytorch.org/whl/cu128 && \
    pip cache purge

# Core Python tooling and runtime libraries in one layer
RUN --mount=type=cache,target=/root/.cache/pip \
...
RUN cd /home/Ubuntu/ComfyUI/custom_nodes && \
    ...
    \
    # Install requirements in a separate step to optimize caching
    for repo_dir in */; do \
